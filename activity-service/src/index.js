...
// รับ event จาก services อื่น — ไม่ต้องมี JWT
...
async function start() {
  ...
  while (retries > 0) {
    try {
      ...
      `);
      console.log('[activity-service] DB connected and tables ready');
      break;
    } catch (e) {
      console.log(`[activity-service] Waiting DB... (${retries} left)`);
      retries--;
      await new Promise(r => setTimeout(r, 3000));
    }
  }
  if (retries === 0) {
    console.error('[activity-service] Could not connect to DB, exiting');
    process.exit(1);
  }
